 ...
 * - Repassar os erros "conhecidos" do Fastify e dos plugins (401, 429...) no formato { error, message }
  ...
  if (hasZodFastifySchemaValidationErrors(error)) {
    return rep.status(400).send({
      ...
      issues: error.validation.map((issue) => ({
        // "/body/target_url" vira "body.target_url", que é mais fácil de ler no front
